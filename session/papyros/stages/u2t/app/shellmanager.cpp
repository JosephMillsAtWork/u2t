    ...
    ,d(new ShellManagerPrivate)
{
    ...
    connect(&unity2dConfiguration(), SIGNAL(formFactorChanged(const QString&)),
                                     SLOT(onFormFactorChanged(const QString&)), Qt::QueuedConnection);
    ...
    onFormFactorChanged(unity2dConfiguration().property("formFactor").toString());
...
void
ShellManager::onFormFactorChanged(const QString& formFactor)
{
    updateDashAlwaysFullScreen();

    disconnect(d->m_superHotModifier, SIGNAL(tapped()), 0, 0);
    if (formFactor == "desktop") {
        connect(d->m_superHotModifier, SIGNAL(tapped()), SLOT(toggleDashRequested()));
    } else if (formFactor == "tv") {
        // FIXME: this should only activate hud in TV mode
        connect(d->m_superHotModifier, SIGNAL(tapped()), SLOT(toggleDashRequested()));
