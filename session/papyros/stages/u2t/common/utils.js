...
.pragma library
var now = new Date()
var time2 = new Date(new Date().getTime() + (24 * 60 * 60 * 1000))
...
function mythNaming(uri) {
    return uri.substring(uri.lastIndexOf("/") +1)
}

function mythFileName(uri){
    var port = ":6543/"
    return encodeURI(uri.substring(uri.lastIndexOf(port)+port.length));
}

function mythUrlstring(uri) {
    return uri.substring(uri.lastIndexOf("@") +1)
}

function uriToNameApps(uri){
    var  apps = "application://"
    return encodeURI(uri.substring(uri.lastIndexOf(apps)+apps.length));
}


function localMusicTest(uri){
  var forLocalMusic = uri.substring(uri.indexOf(":")+1)
}

function avgBgSplit(uri){
//var avgBgl = (unity2dConfiguration.averageBgColor)
Qt.resolvedUrl(uri.substring(uri.lastIndexOf("#")+1))
}

function removeMythPort() {
    return string(0, lastIndexOf(":6543/"))
}
